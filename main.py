...
class MY_GUI():
    ...
    def set_init_window(self):
        ...
        #标签
        self.init_data_label = Label(self.init_window_name, text="操作命令")
        self.init_data_label.grid(row=3, column=0)
        self.init_data_label = Label(self.init_window_name, text="手机号：")
        self.init_data_label.grid(row=0, column=0, sticky = E)
        self.init_data_label = Label(self.init_window_name, text="校区：")
        ...
        self.log_data_Text = Text(self.init_window_name, width=66, height=9, relief=RIDGE)#, state=DISABLED, bd=1)  # 日志框
        self.log_data_Text.grid(row=13, column=0, columnspan=10)
        #按钮
        self.str_trans_to_md5_button = Button(self.init_window_name, text="保存", bg="lightblue", width=10,command=self.filewrite)  # 保存基本信息
        self.str_trans_to_md5_button.grid(row=0, column=3)
        self.str_trans_to_md5_button = Button(self.init_window_name, text="每日健康", bg="lightblue", width=10, command=self.meirijiankang)  # 填报每日健康
        self.str_trans_to_md5_button.grid(row=4, column=0)
        self.str_trans_to_md5_button = Button(self.init_window_name, text="体温填报", bg="lightblue", width=10,command=self.tiwentianbao)  # 填报体温信息
        self.str_trans_to_md5_button.grid(row=4, column=1)
        self.str_trans_to_md5_button = Button(self.init_window_name, text="每日健康+体温填报", bg="lightblue", width=20,command=self.jihe)  # 填报每日健康+体温信息
        self.str_trans_to_md5_button.grid(row=5, column=0, columnspan=2)

    #功能函数
    def fileread(self):

        with open("data.txt",mode='r+',encoding='utf-8') as f:  # 设置文件对象
            data = f.read().split(",")
    ...
    def filewrite(self):
        ...
        f.write(self.init_phone_Text.get("0.0",END).strip()+","+self.init_xiaoqu_Text.get("0.0",END).strip())  # 将字符串写入文件中
        f.close()
